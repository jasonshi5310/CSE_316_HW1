...
class TodoListView {
     ...
     * @param {Boolean} ifLast If current is the last item in the list
     */
    buildListItem(listItem, listItemIndex, ifLast) {
        ...
        dueDateHeaderDiv.setAttribute(TodoHTML.CLASS, TodoGUIClass.LIST_ITEM_CARD_DUE_DATE);
        dueDateHeaderDiv.innerHTML = listItem.getDueDate();
        ...
        newItemDiv.appendChild(dueDateHeaderDiv);
    ...
    loadItems(listToLoad) {
        ...
        for (let i = 0; i < listToLoad.items.length; i++) {
            ...
            let ifLast = false;
            if (i == listToLoad.items.length-1) ifLast = true;
            let itemCard = this.buildListItem(item, i, ifLast);
            listItemsDiv.appendChild(itemCard);
        }

        let addNewItemBtn = document.createElement(TodoHTML.DIV);
        addNewItemBtn.innerHTML = "+";
        addNewItemBtn.setAttribute(TodoHTML.CLASS, TodoGUIClass.LIST_ITEM_CARD);
        addNewItemBtn.id = TodoGUIId.LIST_ITEM_ADD_NEW_ITEM;
        //addNewItemBtn.addEventListener(TodoHTML.CLICK, TodoCallback.PROCESS_ADD_NEW_ITEM);
        $(document).on(TodoHTML.CLICK,'#'+TodoGUIId.LIST_ITEM_ADD_NEW_ITEM, function(){
            window.todo.controller.processAddNewItem();
        });
        listItemsDiv.appendChild(addNewItemBtn);
    }
