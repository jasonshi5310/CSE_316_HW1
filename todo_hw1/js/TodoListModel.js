...
class TodoListModel {
     ...
     * 
     * @param {TodoList} listBeingEdited List in the process of being edited.
     * @param {String} newOnwer The new owner of the list.
     */
    updateListOwner(listBeingEdited, newOnwer) {
        // WE'RE GOING TO CHANGE THE OWNER TOO BUT ONLY UPDATE
        // THE LIST OF LIST LINKS IF IT'S CHANGED
        if (listBeingEdited.getOwner() != newOnwer) {
            listBeingEdited.setOwner(newOnwer);
            this.view.loadListLinks(this.todoLists);
        }
    }
    ...
    /**
     *  Pop a block and let the user confirm if they want to 
     *  delete the list.
     */
    deleteConfirm()
    {
        //$('#list_delete_confirmation_content').addClass('list_dialog_slide_in');
        $("#"+TodoGUIId.LIST_DELETE_CONFIRMATION).removeClass().addClass(TodoGUIId.LIST_DIALOG_SLIDE_IN);

         this.view.showElementWithId(TodoGUIId.LIST_DELETE_CONFIRMATION, true);
       
    }


    /**
     *  Cancel the deletion for list and simply close the dialog
     */
    cancalListDeletion()
    {
        // remove slide_in class and add slide_out class
        $("#"+TodoGUIId.LIST_DELETE_CONFIRMATION).removeClass().addClass(TodoGUIId.LIST_DIALOG_SLIDE_OUT);
        window.setTimeout(() => this.view.showElementWithId(TodoGUIId.LIST_DELETE_CONFIRMATION, false), 500);
        // this.view.showElementWithId(TodoGUIId.LIST_DELETE_CONFIRMATION, false);
    }

    /**
     *  Delete the list and go back to the homepage
     */
    deleteList()
    {
        $("#"+TodoGUIId.LIST_DELETE_CONFIRMATION).addClass(TodoGUIId.LIST_DIALOG_SLIDE_OUT).removeClass(TodoGUIId.LIST_DIALOG_SLIDE_IN);
        window.setTimeout(() => this.view.showElementWithId(TodoGUIId.LIST_DELETE_CONFIRMATION, false), 500);
        this.goHome();
        this.removeList(this.listToEdit);
    }

    /**
     *  Pop a page and let the user input the info for the new item
     */
    addNewItem()
    {
        this.view.showElementWithId(TodoGUIId.TODO_LIST, false);
        this.view.showElementWithId(TodoGUIId.ADD_NEW_ITEM_PAGE, true);
    }
    ...
    cancelNewItem()
    {
        ...
        this.view.showElementWithId(TodoGUIId.TODO_LIST, true);
        this.view.showElementWithId(TodoGUIId.ADD_NEW_ITEM_PAGE, false);
    }

    /**
     *  Sumbit and save the new item in the list
     */
    submitNewItem()
    {
        ...
        this.listToEdit.items.push(newItem);
        this.view.loadItems(this.listToEdit);
        this.cancelNewItem();
    }
    ...
    /**
     * prepare the page to edit the the item
     * 
     * @param {int} index 
     */
    editItem(index)
    {
        var item = this.listToEdit.getItemAtIndex(index);
        document.getElementById(TodoGUIId.ITEM_DESCRIPTION_TEXTFIELD).value = item.getDescription();
        document.getElementById(TodoGUIId.ITEM_ASSIGNED_TO_TEXTFIELD).value = item.getAssignedTo();
